...
import { User, Lock, ArrowLeft, ShieldCheck, LogIn, AlertCircle, Loader2 } from 'lucide-react';
import { loginCustomer } from '../services/api';
import { setSession } from '../utils/session';

export default function CustomerLoginPage() {
  const navigate = useNavigate();
  const [identifier, setIdentifier] = useState('customer@bank.com');
  ...
  const handleLogin = async (e) => {
    ...
    if (result.success && result.data) {
      setSession(result.data.user, 'customer');
      navigate('/customer');
    } else {
